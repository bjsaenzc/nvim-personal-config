...
> **RESOLVED (2026-08-16, later the same day):** migration completed on branch `feature/treesitter-main-migration`. Both nvim-treesitter and nvim-treesitter-textobjects are on `main`; the spec uses `install()` + a FileType autocmd (`vim.treesitter.start()` + treesitter `indentexpr`), rustaceanvim's `ensure_installed` sub-spec was absorbed into the main parser list (rust/toml/ron), and ts-autotag was verified standalone (legacy module only behind a `pcall`). All AC probes pass: highlighting + indent active in lua/python/rust/tsx/markdown, demicolon resolves the new-API `repeatable_move` module, render-markdown and ts-autotag load. Behavior change: `auto_install = true` no longer exists — new languages must be added to `ensure_installed` or `:TSInstall`ed.
>
> Original blocker note kept for history:
...
| 2026-08-16 | P2-11 revisited — treesitter `main` migration (branch `feature/treesitter-main-migration`) | All AC pass: hl+indent active in 5 langs; demicolon on new API; render-markdown/ts-autotag OK | **58.3 (warm median)** | **13/62** |
